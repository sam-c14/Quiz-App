            ...
            <div class="flex h-1/6 mb-5 w-10/12 justify-end">
                <div class="w-full flex flex-wrap justify-end">
                    <div class="w-10/12 py-2 text-end">
                        <label for="">Filter By Alphabet</label>
                    </div>
                    <div class="w-1/4 relative">
                        <input @click="isListShowing = !isListShowing" type="text" v-model="search" name="search-quiz" class="border border-black rounded-sm w-full uppercase font-normal text-sm bg-slate-200 py-1 pl-2" />
                        <div v-if="isListShowing" class="absolute z-20 pt-1 right-1 shadow border top-1 backdrop-blur bg-transparent w-full">
                            <div class="pl-3 hover:bg-blue-500 hover:text-white transition-all font-medium uppercase pb-1 pt-1 text-sm" @click="searchItem" :data-id="item" v-for="item in options" :key="item">{{ item}}</div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="flex justify-center flex-wrap w-full">
                <div v-for="(item, index) in tempQuizImagesArray.length" :key="item"
                    ...
                    <div class="w-full h-full">
                        <img class="h-full w-full" :src="getImageUrl(tempQuizImagesArray[index])" alt="">
                    </div>
...
export default {
    ...
    data() {
        return {
            ...
            count : 0,
            isLoading : false,
            options: ["arts", "sports","music","movie","food"],
            search : '',
            isListShowing : false,
            quizImagesObject: [{ arts: "Art_Quiz_5.jpg" }, { sports: "Golf_Quiz.jpg" }, { music: "History_Quiz_2.jpg" }, { history: "History_Quiz.jpg" }, { movies: "Movie_Quiz_4.jpg" }, { general_knowledge: "OIP.jpg" }, { science: "Science_Quiz_2.jpg" }, { food: "Food_Quiz_3.jpg" }, { geography: "Geography_Quiz.jpg" }],
            tempQuizImagesArray : [],
            ind : 0
        }
    },
    beforeMount() {
        ...
        if (!this.hasBeenMounted){
            ...
            localStorage.setItem("hasBeenMounted", JSON.stringify(this.hasBeenMounted)) // Stores Boolean value to keep track of firstMount
        }
    ...
    methods : {
        ...
        searchItem(e){
            ...
            let temp = []
            temp = this.quizImagesObject.filter(obj=>{
