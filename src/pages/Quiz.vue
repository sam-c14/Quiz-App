<template>
  <div style="background-color: #007faa" class="relative">
    ...
    <section class="grid place-items-center">
      <div
        class="w-2/3 text-white my-2"
        v-for="(obj, index) in tempQuestionArr"
        :key="index"
      >
        <p>{{ obj.id }}. {{ obj.question }}</p>
        <div class="flex justify-between sm:flex-row flex-col flex-wrap">
          <div
            v-for="(option, ind) in obj.options"
            ref="options"
            :id="obj.id + ind"
            :key="ind + parseInt(obj.id)"
            class="sm:w-1/2 w-full my-2 py-1 pb-2 border border-transparent px-1 hover:border-white border-dashed"
            @click="selectOption"
          >
            <span
              ><input type="radio" class="w-6 h-3" :name="obj.question"
            /></span>
      ...
      <aside class="flex justify-between w-10/12">
        <div>
          <div
            class="border border-blue-200 shadow-sm p-0.5 update-btn rounded-md border-opacity-0 hover:border-opacity-100"
          >
            <button
  ...
  <ResultModal v-if="endQuizSession">
    ...
    <template #main>
      <div class="text-center my-4 flex justify-center">
        <div
          class="border happy-text text-white text-sm bg-blue-500 rounded-full p-2 w-1/3 py-7"
        >
          Yayy
        </div>
      </div>
    ...
    <template #footer>
      <button
        @click="
          changeRoute('result', {
            score: result,
            resultArr: JSON.stringify(resultData),
          })
        "
        ...
        >Click Here to see the results</button
      >
    </template>
  </ResultModal>
</template>
      ...
      this.$refs.options.map((option) => {
        ...
        if (this.resultAnalysis[option.id] !== undefined) {
          if (this.resultAnalysis[option.id]) {
            option.firstElementChild.firstElementChild.setAttribute(
              "checked",
              "true"
            );
            ...
            option.firstElementChild.firstElementChild.removeAttribute(
              "checked",
              "true"
            );
  ...
  methods: {
    updateStorage() {
      if (this.isUpdateBtnClicked) {
        var attemptedQuestions = JSON.parse(
          sessionStorage.getItem("attemptedQuestions")
        );
        if (attemptedQuestions == null && this.endQuizSession)
          attemptedQuestions = [];
    ...
    selectOption(e) {
      const tags = [
        ...
        e.currentTarget.parentElement.lastElementChild.previousElementSibling
          .firstElementChild.firstElementChild,
        e.currentTarget.parentElement.lastElementChild.firstElementChild
          .firstElementChild,
      ];
      for (let tag of tags) {
    ...
    renderPreviousPage() {
      if (this.count - 5 !== 0) {
        ...
        this.tempQuestionArr = [
          ...this.questions.slice(startIndex, this.count - 5),
        ];
        this.count -= 5;
    ...
    renderNextPage() {
      if (this.count !== this.questions.length) {
        this.tempQuestionArr = [
          ...this.questions.slice(this.count, this.count + 5),
        ];
        this.count += 5;
        window.scrollTo(0, 0);
      }
    ...
    filterOptions(id) {
      this.selectedOptions = this.selectedOptions.filter((options) => {
        if (!options.startsWith(id[0])) return options;
          ...
          if (
            options.length !== id.length ||
            !options.startsWith(id.slice(0, 2))
          )
            return options;
        }
      });
      ...
      this.questions.map((question) => {
        ...
        if (questionId.includes(question.id)) {
          ...
          if (question.options[answers[index]] == question.answer) {
            ...
            this.resultAnalysis[
              `${question.id}${question.options.indexOf(question.answer)}`
            ] = true;
          }
        } else {
          this.resultData.push({
